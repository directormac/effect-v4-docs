...
- [utils](#utils)
  - [ReadonlyRecord (namespace)](#readonlyrecord-namespace)
    - [NonLiteralKey (type alias)](#nonliteralkey-type-alias)
    - [IntersectKeys (type alias)](#intersectkeys-type-alias)
...
assert.deepStrictEqual(Record.difference({ a: 1, b: 2 }, { b: 3, c: 4 }), { a: 1, c: 4 })
```
...
```ts
declare const makeReducerIntersection: <K extends string, A>(
...
```ts
declare const fromEntries: <Entry extends readonly [string | symbol, any]>(
...
```ts
declare const fromIterableBy: <A, K extends string | symbol>(
  items: Iterable<A>,
  f: (a: A) => K
...
```ts
declare const fromIterableWith: {
...
const x = { a: 1, b: 2, c: 3 }
assert.deepStrictEqual(Record.toEntries(x), [
  ["a", 1],
  ["b", 2],
  ["c", 3]
])
...
```ts
declare const getFailures: <K extends string, A, E>(
...
```ts
declare const getSomes: <K extends string, A>(
...
assert.deepStrictEqual(Record.separate({ a: Result.fail("e"), b: Result.succeed(1) }), [{ a: "e" }, { b: 1 }])
```

**Signature**

```ts
declare const separate: <K extends string, A, B>(
...
```ts
declare const isEmptyReadonlyRecord: <K extends string, A>(
  self: ReadonlyRecord<K, A>
...
assert.deepStrictEqual(
  Record.every({ a: 1, b: 2 }, (n) => n > 0),
  true
)
assert.deepStrictEqual(
  Record.every({ a: 1, b: -1 }, (n) => n > 0),
  false
)
```
...
console.log(isSubrecord(required, available)) // true
console.log(isSubrecord({ role: "Admin", status: "inactive" }, available)) // false
console.log(isSubrecord(required, { role: "editor", status: "active" })) // false
```

**Signature**

```ts
declare const isSubrecordBy: <A>(equivalence: Equivalence<A>) => {
...
assert.deepStrictEqual(
  Record.some({ a: 1, b: 2 }, (n) => n > 1),
  true
)
assert.deepStrictEqual(
  Record.some({ a: 1, b: 2 }, (n) => n > 2),
  false
)
```
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Record.ts#L67)
...
```ts
type NonLiteralKey<K> = K extends string ? (IsFiniteString<K> extends true ? string : K) : symbol
```
...
```ts
type IntersectKeys<K1, K2> = [string] extends [K1 | K2] ? NonLiteralKey<K1> & NonLiteralKey<K2> : K1 & K2
```
