...
- [models](#models)
  - [TxRef (interface)](#txref-interface)
...
const program = Effect.gen(function* () {
  ...
  const value = yield* Effect.tx(TxRef.get(counter))
...
const program = Effect.gen(function* () {
  ...
  yield* Effect.tx(TxRef.set(counter, 100))
...
const program = Effect.gen(function* () {
  ...
  yield* Effect.tx(TxRef.update(counter, (current) => current * 2))
