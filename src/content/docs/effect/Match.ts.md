...
declare const typeTags: {
  <I, Ret>(): <
    P extends { readonly [Tag in Types.Tags<"_tag", I> & string]: (_: Extract<I, { readonly _tag: Tag }>) => Ret } & {
...
const describeValue = Match.type<unknown>().pipe(
...
const describeTruthiness = Match.type<unknown>().pipe(
  Match.when(Match.boolean, (bool) => (bool ? "Definitely true" : "Definitely false")),
...
const processValue = Match.type<string | number | null | undefined>().pipe(
...
const handleValue = Match.type<unknown>().pipe(
  Match.when(Match.instanceOf(CustomError), (err) => `Custom error: ${err.message} (code: ${err.code})`),
  Match.when(Match.instanceOf(Error), (err) => `Standard error: ${err.message}`),
  Match.when(Match.instanceOf(Array), (arr) => `Array with ${arr.length} items`),
  Match.when(Match.instanceOf(Map), (map) => `Map with ${map.size} entries`),
...
const handleStatus = Match.type<string | number>().pipe(
...
const processInput = Match.type<string>().pipe(
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Match.ts#L2059)
...
Match.value({ name: "Alice", age: 30, role: "admin" as const }).pipe(
  Match.when(
    ...
    (user: { name: string; age: number; role: "admin" }) => `Admin: ${user.name}`
  ),
...
type Result = Match.Types.ApplyFilters<string | number | boolean, Match.Types.Only<string>>
// Result: string

type ExclusionResult = Match.Types.ApplyFilters<string | number | boolean, Match.Types.Without<string>>
// Result: number | boolean
```
...
type CustomTags = Match.Types.Tags<
  "type",
  { type: "user"; name: string } | { type: "admin"; permissions: Array<string> }
>
// Result: "user" | "admin"
```
...
type Combined = Match.Types.ArrayToIntersection<[{ name: string }, { age: number }, { active: boolean }]>
...
type StringExtract = Match.Types.ExtractMatch<string | number | boolean, typeof Match.string>
// Result: string
