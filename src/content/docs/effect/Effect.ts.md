...
- [error handling](#error-handling)
  - [TagsWithReason (type alias)](#tagswithreason-type-alias)
  - [catch](#catch)
...
- [utils](#utils)
  - [All (namespace)](#all-namespace)
    ...
    - [IsResult (type alias)](#isresult-type-alias)
    - [Return (type alias)](#return-type-alias)
  - [Effectify (namespace)](#effectify-namespace)
    - [Effectify (type alias)](#effectify-type-alias)
    - [EffectifyError (type alias)](#effectifyerror-type-alias)
  - [Repeat (namespace)](#repeat-namespace)
    - [Options (interface)](#options-interface)
    - [Return (type alias)](#return-type-alias-1)
  - [Retry (namespace)](#retry-namespace)
    - [Options (interface)](#options-interface-1)
    - [Return (type alias)](#return-type-alias-2)
  - [fn (namespace)](#fn-namespace)
    - [Return (type alias)](#return-type-alias-3)
    - [Untraced (type alias)](#untraced-type-alias)
    - [Traced (type alias)](#traced-type-alias)
  - [gen (namespace)](#gen-namespace)
    - [Return (type alias)](#return-type-alias-4)
- [zipping](#zipping)
  - [zip](#zip)
  - [zipWith](#zipwith)
...
const program = Effect.partition([0, 1, 2, 3], (n) => (n % 2 === 0 ? Effect.fail(`${n} is even`) : Effect.succeed(n)))
...
const program = Effect.when(Console.log("Condition is true!"), Effect.succeed(shouldLog))
...
//      ▼
const failure = Effect.fail(new OperationFailedError())
```
...
const program = Effect.failCause(Cause.fail("Network error"))
...
const program = Effect.failCauseSync(() => Cause.fail("Error computed at runtime"))
...
const applyDiscount = (total: number, discountRate: number): Effect.Effect<number, DiscountRateError> =>
  discountRate === 0 ? Effect.fail(new DiscountRateError()) : Effect.succeed(total - (total * discountRate) / 100)
...
const parseJSON = (input: string) => Effect.try(() => JSON.parse(input))
...
```ts
declare const tryPromise: <A, E = Cause.UnknownError>(
  options:
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Effect.ts#L1162)
...
const program = Effect.service(Config).pipe(Effect.map((config) => `Hello ${config.name}!`))

// Transform services by providing Config while keeping Logger requirement
const configured = program.pipe(
  Effect.updateContext((context: Context.Context<typeof Logger>) => Context.add(context, Config, { name: "World" }))
)
...
}).pipe(Effect.updateService(Counter, (counter) => ({ count: counter.count + 1 })))
...
```ts
declare const fromOption: <Arg extends Option<unknown> | LazyArg<unknown>, E = Cause.NoSuchElementError>(
  arg: Arg,
...
const divide = (a: number, b: number) => (b === 0 ? Effect.fail(new DivideByZeroError()) : Effect.succeed(a / b))
...
const program = Effect.delay(Console.log("Delayed message"), "1 second")
...
declare const timeoutOrElse: {
  ...
  <A, E, R, A2, E2, R2>(
    self: Effect<A, E, R>,
    options: { readonly duration: Duration.Input; readonly orElse: LazyArg<Effect<A2, E2, R2>> }
...
const recovered = Effect.catchEager(failed, (err: string) => Effect.succeed(`recovered from: ${err}`)) // Applied eagerly

// For success effects, returns success as-is
const success = Effect.succeed(42)
const unchanged = Effect.catchEager(success, (err: string) => Effect.succeed(`recovered from: ${err}`)) // Returns success as-is

// For pending effects, behaves like regular catch
const pending = Effect.delay(Effect.fail("error"), "100 millis")
const recoveredPending = Effect.catchEager(pending, (err: string) => Effect.succeed(`recovered from: ${err}`)) // Uses regular catch
```
...
const context = Context.make(Logger, { log: console.log }).pipe(Context.add(Database, { query: () => "result" }))
...
const program = Effect.validate([0, 1, 2, 3], (n) => (n % 2 === 0 ? Effect.fail(`${n} is even`) : Effect.succeed(n)))
...
declare const validate: {
  <A, B, E, R>(
    f: (a: A, i: number) => Effect<B, E, R>,
    options?: { readonly concurrency?: Concurrency | undefined; readonly discard?: false | undefined } | undefined
...
const recovered = Effect.catchCause(program, (cause) => {
  if (Cause.hasDies(cause)) {
    return Console.log("Caught defect").pipe(Effect.as("Recovered from defect"))
  }
...
const program = Effect.catchCauseIf(httpRequest, Cause.hasFails, (cause) =>
  Effect.gen(function* () {
...
const recovered = Effect.catchDefect(program, (defect) => {
  return Console.log(`Caught defect: ${defect}`).pipe(Effect.as("Recovered from defect"))
})
```
...
const recovered2 = program.pipe(
  Effect.catchFilter(Filter.tagged("NotFound"), (error) => Effect.succeed(`missing:${error.id}`))
)
```
...
const handled = program.pipe(
  Effect.catchReason("AiError", "RateLimitError", (reason) => Effect.succeed(`Retry after ${reason.retryAfter}s`))
)
```
...
declare const ignore: <
  Arg extends
    | Effect<any, any, any>
    | { readonly log?: boolean | Severity | undefined; readonly message?: string | undefined }
    | undefined = { readonly log?: boolean | Severity | undefined; readonly message?: string | undefined }
>(
  effectOrOptions?: Arg,
  options?: { readonly log?: boolean | Severity | undefined; readonly message?: string | undefined } | undefined
...
//      ▼
const mapped = Effect.mapError(simulatedTask, (message) => new TaskError({ message }))
```
...
const program = Effect.retryOrElse(networkRequest, Schedule.recurs(2), (error, retryCount) =>
  Effect.gen(function* () {
...
const program = Effect.firstSuccessOf([primary, secondary, tertiary])
...
class Counter {
  ...
  increment = Effect.fn("Counter.increment")({ self: this }, function* (this: Counter, by: number) {
    this.count += by
...
const f = Effect.fn("calculateLength")(function* (value: string): Effect.fn.Return<number> {
  return yield* Effect.succeed(value.length)
})
...
const f = Effect.fn("succeed")(function* <A>(value: A): Effect.fn.Return<A> {
...
const f = Effect.fnUntraced(function* (value: string) {
  return yield* Effect.succeed(value.length)
})
...
const f = Effect.fnUntraced(function* (value: string): Effect.fn.Return<number> {
  return yield* Effect.succeed(value.length)
})
...
const f = Effect.fnUntraced(function* <A>(value: A): Effect.fn.Return<A> {
  return yield* Effect.succeed(value)
})
...
const program = Effect.onInterrupt(task, () => Console.log("Task was interrupted, cleaning up..."))
...
```ts
declare const withLogSpan: ((label: string) => <A, E, R>(effect: Effect<A, E, R>) => Effect<A, E, R>) &
...
const customLogger = Logger.make(({ message }) => Effect.sync(() => console.log(`[CUSTOM]: ${message}`)))
...
```ts
declare const withLogger: (<Output>(
...
const failure: Effect.Effect<number, ExampleError> = Effect.fail(new ExampleError({ message: "Uh oh!" }))
...
declare const matchCauseEager: {
  ...
  <A, E, R, A2, A3>(
    self: Effect<A, E, R>,
    options: { readonly onFailure: (cause: Cause.Cause<E>) => A2; readonly onSuccess: (value: A) => A3 }
  ): Effect<A2 | A3, never, R>
}
```
...
declare const forever: <
  Arg extends Effect<any, any, any> | { readonly disableYield?: boolean | undefined } | undefined = {
...
const program = Effect.repeatOrElse(task, Schedule.recurs(3), (error, attempts) =>
  Console.log(`Final failure: ${error}, after ${Option.getOrElse(attempts, () => 0)} attempts`).pipe(
    Effect.map(() => 0)
  )
)
```
...
const program = Effect.scheduleFrom(task(0), 0, Schedule.recurs(2))
...
const program = Effect.scoped(
  Effect.gen(function* () {
    ...
    const db = yield* Effect.acquireDisposable(Effect.sync(() => new sqlite.DatabaseSync(":memory:")))
...
const program = Effect.ensuring(task, Console.log("Cleanup: This always runs!"))
...
const program = Effect.onError(task, (cause) => Console.log(`Cleanup on error: ${Cause.squash(cause)}`))
...
const program = Effect.onErrorIf(task, Cause.hasFails, (cause) =>
  Effect.gen(function* () {
...
const program = Effect.onExitIf(Effect.succeed(42), Exit.isSuccess, (exit) =>
  Exit.isSuccess(exit) ? Console.log(`Succeeded with: ${exit.value}`) : Effect.void
)
```
...
const program = Effect.gen(function* () {
  ...
  const resource = yield* Effect.acquireRelease(Console.log("Acquiring resource").pipe(Effect.as("resource")), () =>
...
const resource = Effect.acquireRelease(Console.log("Acquiring resource").pipe(Effect.as("resource")), () =>
  Console.log("Releasing resource")
)
...
const program = Effect.scopedWith((scope) =>
  Effect.gen(function* () {
    ...
    const resource = yield* Effect.scoped(
      Effect.acquireRelease(Console.log("Acquiring resource").pipe(Effect.as("resource")), () =>
        Console.log("Releasing resource")
      )
    )
...
```ts
declare const runCallbackWith: <R>(
...
const program = Effect.tapCause(task, (cause) => Console.log(`Logging cause: ${Cause.squash(cause)}`))
...
const program = Effect.tapCauseIf(task, Cause.hasFails, (cause) =>
...
const task2: Effect.Effect<number> = Effect.die("Something went wrong")
...
const task: Effect.Effect<number, NetworkError | ValidationError> = Effect.fail(new NetworkError({ statusCode: 504 }))

const program = Effect.tapErrorTag(task, "NetworkError", (error) => Console.log(`expected error: ${error.statusCode}`))
...
declare const forkChild: <
  Arg extends
    | Effect<any, any, any>
    | { readonly startImmediately?: boolean | undefined; readonly uninterruptible?: boolean | "inherit" | undefined }
    | undefined = {
...
const parentEffect = Effect.withSpan("parent-operation")(Effect.succeed("parent result"))

const childEffect = Effect.withSpan("child-operation")(Effect.succeed("child result"))

// Link the child span to the parent span
const program = Effect.gen(function* () {
  const parentSpan = yield* Effect.currentSpan
  const result = yield* childEffect.pipe(Effect.linkSpans(parentSpan, { relationship: "follows" }))
  return result
})
```
...
const program = Effect.succeed("Hello").pipe(Effect.track(counter))
...
const effect = Effect.succeed("result").pipe(Effect.track(exitTracker, mapExitToString))
```
...
const defectCounter = Metric.counter("defects").pipe(Metric.withConstantInput(1))

const program = Effect.die("Critical system failure").pipe(Effect.trackDefects(defectCounter))
...
const program = Effect.sleep("100 millis").pipe(Effect.trackDuration(executionTimer))
...
const errorCounter = Metric.counter("errors").pipe(Metric.withConstantInput(1))

const program = Effect.fail("Network timeout").pipe(Effect.trackErrors(errorCounter))
...
const successCounter = Metric.counter("successes").pipe(Metric.withConstantInput(1))

const program = Effect.succeed(42).pipe(Effect.trackSuccesses(successCounter))
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Effect.ts#L90)
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Effect.ts#L82)
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Effect.ts#L260)
...
```ts
type ReturnIterable<T, Discard, Mode> = [T] extends [Iterable<Effect<infer A, infer E, infer R>>]
  ? Effect<
...
```ts
type ReturnObject<T, Discard, Mode> = [T] extends [Record<string, EffectAny>]
  ? Effect<
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Effect.ts#L14745)
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Effect.ts#L7392)
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Effect.ts#L3944)
...
type Untraced = {
  ...
  <Self, Eff extends Effect<any, any, any>, AEff, Args extends Array<any>, A, B, C, D, E, F, G, H, I, J, K, L, M, N, O>(
    body: (this: Self, ...args: Args) => Generator<Eff, AEff, never>,
    a: (
      _: Effect<
        AEff,
  ...
  <Eff extends Effect<any, any, any>, AEff, Args extends Array<any>, A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, Q>(
    body: (this: unassigned, ...args: Args) => Generator<Eff, AEff, never>,
    a: (
      _: Effect<
        AEff,
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Effect.ts#L1438)
