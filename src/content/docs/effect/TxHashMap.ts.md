...
- [models](#models)
  - [TxHashMap (interface)](#txhashmap-interface)
- [utils](#utils)
  - [TxHashMap (namespace)](#txhashmap-namespace)
...
const program = Effect.gen(function* () {
  const config = yield* TxHashMap.make(["host", "localhost"], ["port", "3000"], ["ssl", "false"])
...
const program = Effect.gen(function* () {
  ...
  const allCanRead = yield* TxHashMap.every(permissions, (perms) => perms.canRead)
  console.log(allCanRead) // true

  // Check if all users can write
  const allCanWrite = yield* TxHashMap.every(permissions, (perms) => perms.canWrite)
...
const program = Effect.gen(function* () {
  ...
  // Data-last usage with pipe
  const expensiveItems = yield* inventory.pipe(TxHashMap.filter((product) => product.price > 500))
...
const program = Effect.gen(function* () {
  ...
  const activeAdminAges = yield* TxHashMap.filterMap(userData, (user, username) => {
...
const program = Effect.gen(function* () {
  ...
  const highPriorityTask = yield* TxHashMap.findFirst(tasks, (task) => task.priority >= 2 && !task.completed)
  ...
  const aliceTask = yield* tasks.pipe(TxHashMap.findFirst((task) => task.assignee === "alice"))
...
const program = Effect.gen(function* () {
  ...
  const departments = yield* TxHashMap.make(["engineering", ["alice", "bob"]], ["marketing", ["charlie", "diana"]])

  // Expand each department into individual employee entries with metadata
  const employeeDetails = yield* TxHashMap.flatMap(departments, (employees, department) =>
    Effect.gen(function* () {
...
const program = Effect.gen(function* () {
  ...
  yield* logs.pipe(
    TxHashMap.forEach((logInfo) =>
      logInfo.size > 1000 ? Console.log(`Large log file: ${logInfo.size} bytes`) : Effect.void
    )
  )
})
```
...
const program = Effect.gen(function* () {
  const permissions = yield* TxHashMap.make(["alice", ["read", "write"]], ["bob", ["read"]], ["charlie", ["admin"]])
...
const program = Effect.gen(function* () {
  ...
  const hasOnlineUsers = yield* TxHashMap.hasBy(userStatuses, (user) => user.status === "online")
  console.log(hasOnlineUsers) // true

  // Check if any users have specific username pattern
  const hasAdminUser = yield* TxHashMap.hasBy(userStatuses, (user, username) => username.startsWith("admin"))
...
const program = Effect.gen(function* () {
  const userRoles = yield* TxHashMap.make(["alice", "admin"], ["bob", "user"], ["charlie", "moderator"])
...
const program = Effect.gen(function* () {
  ...
  const greetings = yield* TxHashMap.map(profiles, (profile, userId) => `Hello, ${profile.name}! (User: ${userId})`)
  ...
  // Data-last usage with pipe
  const ages = yield* profiles.pipe(TxHashMap.map((profile) => profile.age))
...
const program = Effect.gen(function* () {
  const counters = yield* TxHashMap.make(["downloads", 100], ["views", 250])
...
const program = Effect.gen(function* () {
  ...
  yield* TxHashMap.modifyAt(storage, "access_count", (current) =>
    current._tag === "Some" && typeof current.value === "number" ? { ...current, value: current.value + 1 } : current
  )
...
const program = Effect.gen(function* () {
  ...
  const sales = yield* TxHashMap.make(["Q1", 15000], ["Q2", 18000], ["Q3", 22000], ["Q4", 25000])

  // Calculate total sales
  const totalSales = yield* TxHashMap.reduce(sales, 0, (total, amount, quarter) => {
  ...
  const quarterlyReport = yield* sales.pipe(
    TxHashMap.reduce({ quarters: 0, total: 0, max: 0 }, (report, amount, quarter) => ({
...
const program = Effect.gen(function* () {
  const inventory = yield* TxHashMap.make(["laptop", 5], ["mouse", 20])
...
const program = Effect.gen(function* () {
  const metrics = yield* TxHashMap.make(["requests", 1000], ["errors", 5], ["users", 50])
...
const program = Effect.gen(function* () {
  const liveData = yield* TxHashMap.make(["temperature", 22.5], ["humidity", 45.2], ["pressure", 1013.25])
...
const program = Effect.gen(function* () {
  ...
  const hasExpensiveProducts = yield* TxHashMap.some(inventory, (product) => product.price > 500)
  console.log(hasExpensiveProducts) // true

  // Check if any products are out of stock
  const hasOutOfStock = yield* TxHashMap.some(inventory, (product) => product.stock === 0)
  console.log(hasOutOfStock) // true

  // Data-last usage with pipe
  const hasAffordableItems = yield* inventory.pipe(TxHashMap.some((product) => product.price < 50))
...
const program = Effect.gen(function* () {
  const settings = yield* TxHashMap.make(["theme", "dark"], ["language", "en-US"], ["timezone", "UTC"])
...
const program = Effect.gen(function* () {
  ...
  const userPrefs = yield* TxHashMap.make(["theme", "light"], ["language", "en"], ["notifications", "enabled"])
...
const program = Effect.gen(function* () {
  const scores = yield* TxHashMap.make(["alice", 95], ["bob", 87], ["charlie", 92])
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/TxHashMap.ts#L125)
...
const program = Effect.gen(function* () {
  ...
  const userMap = yield* TxHashMap.make(["alice", { name: "Alice", age: 30 }], ["bob", { name: "Bob", age: 25 }])
...
const program = Effect.gen(function* () {
  ...
  const config = yield* TxHashMap.make(["api_url", "https://api.example.com"], ["timeout", "5000"], ["retries", "3"])
