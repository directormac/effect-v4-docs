...
- [models](#models)
  ...
  - [FilterEffect (interface)](#filtereffect-interface)
...
const positiveNumbers = Filter.fromPredicate((n: number) => n > 0)
const doubleNumbers = Filter.make((n: number) => (n > 0 ? Result.succeed(n * 2) : Result.fail(n)))
...
```ts
declare const equals: <const A, Input = unknown>(
  value: A
...
```ts
declare const equalsStrict: <const A, Input = unknown>(
  value: A
) => Filter<Input, A, EqualsWith<Input, A, A, Exclude<Input, A>>>
```
...
```ts
declare const instanceOf: <K extends new (...args: any) => any>(
  constructor: K
...
const uppercaseFilter = Filter.make((s: string) => (s.length > 0 ? Result.succeed(s.toUpperCase()) : Result.fail(s)))
```
...
```ts
declare const makeEffect: <Input, Pass, Fail, E, R>(
