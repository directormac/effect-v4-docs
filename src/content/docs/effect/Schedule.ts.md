...
- [metadata](#metadata)
  ...
  - [InputMetadata (interface)](#inputmetadata-interface)
...
const inputCollector = Schedule.collectInputs(Schedule.spaced("100 millis"))
...
```ts
declare const collectInputs: <Output, Input, Error, Env>(
...
const outputCollector = Schedule.collectOutputs(Schedule.recurs(4))
...
```ts
declare const collectOutputs: <Output, Input, Error, Env>(
  self: Schedule<Output, Input, Error, Env>
...
const collectWhileSmall = Schedule.collectWhile(Schedule.exponential("100 millis"), (metadata) =>
...
const collectWithCheck = Schedule.collectWhile(Schedule.fixed("1 second"), (metadata) =>
  Effect.gen(function* () {
...
const effectfulProgram = Effect.gen(function* () {
  const results = yield* Effect.repeat(Effect.succeed("checked"), collectWithCheck)
...
const collectSamples = Schedule.collectWhile(Schedule.spaced("200 millis"), (metadata) =>
  Effect.succeed(metadata.attempt <= 5 && metadata.elapsed < 2000)
)
...
const samplingProgram = Effect.gen(function* () {
  ...
  yield* Console.log(`Collected ${collected.length} samples, average: ${average.toFixed(1)}`)
})
```
...
const leftSchedule = Schedule.exponential("100 millis").pipe(Schedule.map(() => Effect.succeed("left-result")))
...
const leftSchedule = Schedule.exponential("100 millis").pipe(Schedule.map(() => Effect.succeed("left-result")))
const rightSchedule = Schedule.spaced("50 millis").pipe(Schedule.map(() => Effect.succeed("right-result")))
...
const leftSchedule = Schedule.exponential("100 millis").pipe(Schedule.map(() => Effect.succeed("left")))
const rightSchedule = Schedule.spaced("50 millis").pipe(Schedule.map(() => Effect.succeed("right")))
...
const program = Effect.gen(function* () {
  const results = yield* Effect.repeat(
    ...
    eitherSchedule.pipe(
      Schedule.tapOutput(([timeResult, countResult]) => Console.log(`Time: ${timeResult}, Count: ${countResult}`))
    )
  )
...
const backupSchedule = Schedule.spaced("500 millis").pipe(Schedule.map(() => Effect.succeed("backup-result")))
...
const fallbackSchedule = Schedule.spaced("500 millis").pipe(Schedule.map(() => Effect.succeed("fallback")))
...
const minutelyProgram = Effect.gen(function* () {
  yield* Effect.repeat(
    ...
    everyMinute.pipe(
      ...
      Schedule.tapOutput((duration) => Console.log(`Next execution in: ${duration}`))
    )
  )
})
...
const complexCron = Schedule.cron("0 2,4,6 * * *").pipe(
  Schedule.tapOutput((duration) => Console.log(`Scheduled to run again in ${duration}`))
)
...
const robustProgram = Effect.gen(function* () {
  ...
  ).pipe(Effect.catch((error: unknown) => Console.log(`Cron task error: ${String(error)}`)))
})
```
...
const exponentialDelays = Schedule.delays(Schedule.exponential("100 millis").pipe(Schedule.take(5)))
...
const delayProgram = Effect.gen(function* () {
  yield* Effect.repeat(
    ...
    exponentialDelays.pipe(Schedule.tapOutput((delay) => Console.log(`Waiting ${delay} before next execution`)))
  )
})

// Monitor delays from a Fibonacci schedule
const fibonacciDelays = Schedule.delays(Schedule.fibonacci("200 millis").pipe(Schedule.take(8)))

const fibDelayProgram = Effect.gen(function* () {
  yield* Effect.repeat(
    Console.log("Fibonacci task"),
    fibonacciDelays.pipe(Schedule.tapOutput((delay) => Console.log(`Fibonacci delay: ${delay}`)))
  )
})

// Extract delays for analysis or logging
const analyzeDelays = Schedule.delays(Schedule.spaced("1 second").pipe(Schedule.take(3))).pipe(
...
const adaptiveSchedule = Schedule.unfold(100, (delay) => Effect.succeed(delay * 1.5)).pipe(Schedule.take(6))
...
const adaptiveProgram = Effect.gen(function* () {
  yield* Effect.repeat(
    ...
    adaptiveDelays.pipe(Schedule.tapOutput((delay) => Console.log(`Adaptive delay: ${delay}`)))
  )
})

// Use delays to implement custom timing logic
const customTimingSchedule = Schedule.delays(Schedule.exponential("50 millis").pipe(Schedule.take(4))).pipe(
...
const program = Effect.repeat(Console.log("runs again after one second"), Schedule.duration("1 second"))
```
...
const timedProgram = Effect.gen(function* () {
  yield* Effect.repeat(
    ...
    fiveSecondSchedule.pipe(Schedule.tapOutput((elapsedDuration) => Console.log(`Total elapsed: ${elapsedDuration}`)))
  )
...
const timedRetry = Schedule.exponential("200 millis").pipe(Schedule.both(Schedule.during("30 seconds")))
...
}).pipe(Effect.catch((error: unknown) => Console.log(`Timed out: ${String(error)}`)))
```
...
const program = Effect.gen(function* () {
  yield* Effect.repeat(
    ...
    Schedule.spaced("1 second").pipe(
      ...
      Schedule.tapOutput(([count, duration]) => Console.log(`Run ${count}, elapsed: ${Duration.toMillis(duration)}ms`)),
      Schedule.take(5)
    )
  )
})
...
const retryPolicy = Schedule.exponential("50 millis").pipe(Schedule.both(Schedule.recurs(5)))
...
const healthCheck = Effect.gen(function* () {
  ...
  return "healthy"
}).pipe(Effect.repeat(Schedule.fixed("2 seconds").pipe(Schedule.take(5))))
...
const fixedSchedule = longRunningTask.pipe(Effect.repeat(Schedule.fixed("1 second").pipe(Schedule.take(3))))

// Comparing with spaced (waits 1s AFTER each task)
const spacedSchedule = longRunningTask.pipe(Effect.repeat(Schedule.spaced("1 second").pipe(Schedule.take(3))))
...
```ts
declare const fromStep: <Input, Output, EnvX, Error, ErrorX, Env>(
...
const firstThreeInputs = Schedule.fromStepWithMetadata(
  Effect.succeed((metadata: Schedule.InputMetadata<string>) => {
    if (metadata.attempt > 3) {
      return Cause.done("finished")
    }

    return Effect.succeed([`attempt ${metadata.attempt}: ${metadata.input}`, Duration.millis(250)] as [
      string,
      Duration.Duration
    ])
  })
...
```ts
declare const fromStepWithMetadata: <Input, Output, EnvX, ErrorX, Error, Env>(
  step: Effect<
...
const countingSchedule = Schedule.recurs(3).pipe(Schedule.tapOutput((count) => Console.log(`Execution #${count + 1}`)))
```
...
const heartbeat = Effect.gen(function* () {
  yield* Console.log("Heartbeat")
}).pipe(Effect.repeat(everyTwoSeconds))
...
const limitedTask = Effect.gen(function* () {
  ...
  return "Task completed"
}).pipe(Effect.repeat(Schedule.spaced("1 second").pipe(Schedule.take(5))))
...
const fibonacciSchedule = Schedule.unfold([0, 1] as [number, number], ([a, b]) =>
  Effect.succeed([b, a + b] as [number, number])
)
...
const adjustedDelaySchedule = Schedule.unfold({ delay: 1000, adjustment: 100 }, ({ delay, adjustment }) =>
  Effect.gen(function* () {
...
```ts
declare const unfold: <State, Error = never, Env = never>(
  initial: State,
...
const delayedSchedule = Schedule.addDelay(Schedule.exponential("100 millis").pipe(Schedule.take(5)), (output) =>
...
const repeatProgram = Effect.gen(function* () {
  yield* Effect.repeat(
    ...
    delayedSchedule.pipe(Schedule.tapOutput((delay) => Console.log(`Base delay: ${delay}`)))
  )
})

// Add adaptive delay based on execution count
const adaptiveSchedule = Schedule.addDelay(Schedule.recurs(6), (executionCount) =>
...
const adaptiveProgram = Effect.gen(function* () {
  yield* Effect.repeat(
    ...
    adaptiveSchedule.pipe(Schedule.tapOutput((count) => Console.log(`Execution ${count + 1} with adaptive delay`)))
  )
})
...
const dynamicSchedule = Schedule.addDelay(Schedule.spaced("1 second").pipe(Schedule.take(4)), (executionNumber) => {
...
```ts
declare const jittered: <Output, Input, Error, Env>(
  self: Schedule<Output, Input, Error, Env>
...
```ts
declare const toStep: <Output, Input, Error, Env>(
...
```ts
declare const toStepWithMetadata: <Output, Input, Error, Env>(
  schedule: Schedule<Output, Input, Error, Env>
...
```ts
declare const toStepWithSleep: <Output, Input, Error, Env>(
  schedule: Schedule<Output, Input, Error, Env>
...
const sumProgram = Effect.gen(function* () {
  const finalSum = yield* Effect.repeat(
    ...
    sumSchedule.pipe(Schedule.tapOutput((sum) => Console.log(`Running sum: ${sum}`)))
  )
...
const configProgram = Effect.gen(function* () {
  const finalConfig = yield* Effect.repeat(
    ...
    configBuilder.pipe(
      Schedule.tapOutput((config) => Console.log(`Config: retries=${config.retries}, timeout=${config.timeout}ms`))
    )
  )
...
const countSchedule = Schedule.recurs(5).pipe(Schedule.map((count) => Effect.succeed(`Execution #${count + 1}`)))
...
const program = Effect.gen(function* () {
  const results = yield* Effect.repeat(
    ...
    structuredSchedule.pipe(
      Schedule.take(8),
      Schedule.tapOutput((info) => Console.log(`${info.phase} phase - iteration ${info.iteration}`))
    )
  )
...
const inputSchedule = Schedule.passthrough(Schedule.exponential("100 millis").pipe(Schedule.take(3)))
...
```ts
declare const passthrough: <Output, Input, Error, Env>(
  self: Schedule<Output, Input, Error, Env>
...
const metadataAwareSchedule = Schedule.spaced("1 second").pipe(
  Schedule.collectWhile((metadata) => Effect.succeed(metadata.attempt <= 5 && metadata.elapsed < 10000))
)

const program = Effect.gen(function* () {
  yield* Effect.repeat(Console.log("Task execution"), metadataAwareSchedule)
})
```
...
const loggingSchedule = Schedule.unfold(0, (n) => Effect.succeed(n + 1)).pipe(
  ...
  Schedule.collectWhile((metadata) =>
    Console.log(`Output: ${metadata.output}, attempt: ${metadata.attempt}, elapsed: ${metadata.elapsed}ms`).pipe(
      Effect.as(metadata.attempt <= 3)
    )
  )
)

const program = Effect.gen(function* () {
  yield* Effect.repeat(Effect.succeed("task completed"), loggingSchedule.pipe(Schedule.take(3)))
})
...
const retrySchedule = Schedule.exponential("100 millis").pipe(Schedule.both(Schedule.recurs(3)))

// Basic repeat schedule - repeat every 30 seconds forever
const repeatSchedule: Schedule.Schedule<number, unknown, never> = Schedule.spaced("30 seconds")
...
const program = Effect.gen(function* () {
  ...
  yield* Console.log("heartbeat").pipe(Effect.repeat(repeatSchedule.pipe(Schedule.take(5))))
})
```
...
const quickRetries = Schedule.exponential("100 millis").pipe(Schedule.take(3))
const slowRetries = Schedule.exponential("1 second").pipe(Schedule.take(2))
...
const program = Effect.retry(Effect.fail("transient error"), monitoredSchedule)
```
...
const errorLoggingSchedule = Schedule.exponential("100 millis").pipe(
  Schedule.take(3),
  Schedule.tapInput((error: RetryError) => Console.log(`Retry triggered by error: ${String(error)}`))
)
...
const alertProgram = Effect.gen(function* () {
  ...
  yield* Effect.retry(
    ...
    alertingSchedule
  ).pipe(Effect.catch((error: unknown) => Console.log(`All retries exhausted: ${String(error)}`)))
})

// Chain multiple input taps for different purposes
const comprehensiveSchedule = Schedule.fibonacci("100 millis").pipe(
  Schedule.take(5),
  Schedule.tapInput((error: RetryError) => Console.log(`Error occurred: ${error._tag}`)),
  Schedule.tapInput((error: RetryError) =>
    String(error).length > 20 ? Console.log("Long error message detected") : Effect.void
  )
)
```
...
const alertingSchedule = Schedule.fibonacci("200 millis").pipe(
  ...
  Schedule.tapOutput((delay) =>
    Effect.gen(function* () {
      ...
      if (delayMs.includes("1000")) {
        // Alert on delays >= 1 second
...
const comprehensiveSchedule = Schedule.fixed("500 millis").pipe(
  ...
  Schedule.tapOutput((count) => (count % 3 === 0 ? Console.log("Checkpoint reached") : Effect.void))
)
```
...
const retryProgram = Effect.gen(function* () {
  ...
  yield* Console.log(`Result: ${result}`)
}).pipe(Effect.catch((error: unknown) => Console.log(`Failed after limited retries: ${String(error)}`)))
...
const samplingProgram = Effect.gen(function* () {
  yield* Effect.repeat(
    ...
    samplingSchedule.pipe(Schedule.tapOutput((label) => Console.log(`Completed: ${label}`)))
  )
})
```
...
```ts
declare const satisfiesErrorType: <T>() => <Error extends T, Output = never, Input = unknown, Env = never>(
  self: Schedule<Output, Input, Error, Env>
) => Schedule<Output, Input, Error, Env>
```
...
```ts
declare const satisfiesInputType: <T>() => <Input extends T, Output = never, Error = never, Env = never>(
  self: Schedule<Output, Input, Error, Env>
) => Schedule<Output, Input, Error, Env>
```
...
```ts
declare const satisfiesOutputType: <T>() => <Output extends T, Error = never, Input = unknown, Env = never>(
  self: Schedule<Output, Input, Error, Env>
) => Schedule<Output, Input, Error, Env>
```
...
```ts
declare const satisfiesServicesType: <T>() => <Env extends T, Output = never, Input = unknown, Error = never>(
  self: Schedule<Output, Input, Error, Env>
) => Schedule<Output, Input, Error, Env>
```
...
const schedule = Schedule.recurs(3).pipe(Schedule.setInputType<string>())
```
...
```ts
declare const setInputType: <T>() => <Output, Error, Env>(
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Schedule.ts#L214)
...
const stringSchedule = Schedule.spaced("1 second").pipe(Schedule.map(() => Effect.succeed("tick")))

// Schedule that only accepts Error inputs
const errorSchedule = Schedule.exponential("100 millis").pipe(Schedule.take(5))
