...
```ts
declare const awaitAll: <A extends Fiber<any, any>>(
  self: Iterable<A>
) => Effect<
...
```ts
declare const joinAll: <A extends Iterable<Fiber<any, any>>>(
  self: A
) => Effect<
...
const program = Effect.gen(function* () {
  const targetFiber = yield* Effect.forkChild(Effect.delay("5 seconds")(Effect.succeed("task completed")))
...
[Source](https://github.com/Effect-TS/effect-smol/tree/main/packages/effect/src/Fiber.ts#L130)
